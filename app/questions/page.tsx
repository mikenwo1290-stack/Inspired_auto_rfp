...
function QuestionsContent() {
  ...
  const handleGenerateAnswer = (questionId: string) => {
    ...
    setTimeout(() => {
      // Generate a mock answer but don't save it yet - QuestionItem will handle this
      const mockAnswer = "This is an auto-generated answer for this question. In the future, this will be generated by an AI model based on the question content.";
      
      // Just update the local state but no auto-save - this will be passed to the component
      // which will manage its own local state and require explicit saving
      handleAnswerChange(questionId, mockAnswer);
  ...
  const handleGenerateAllAnswers = () => {
    if (isGeneratingAll) return;
    
    setIsGeneratingAll(true);
    
    // Create a progress simulation for UI feedback
    const allQuestionIds = rfpDocument?.sections.flatMap(section => section.questions.map(q => q.id)) || [];
    
    // Process questions with a delay to show progress
    let processed = 0;
    const interval = setInterval(() => {
      if (processed >= allQuestionIds.length) {
        clearInterval(interval);
        setIsGeneratingAll(false);
        toast({
          title: "All Answers Generated",
          description: `Generated ${allQuestionIds.length} answers. Please review and save them.`,
        });
        return;
      }
      
      const questionId = allQuestionIds[processed];
      setIsGenerating(prev => ({ ...prev, [questionId]: true }));
      
      // Mock answer for UI demonstration
      const mockAnswer = `This is an auto-generated answer for question #${processed + 1}. In the future, this will use AI to generate custom responses for each question.`;
      
      // Update the answer but do not auto-save - QuestionItem components will handle this
      setTimeout(() => {
        handleAnswerChange(questionId, mockAnswer);
        setIsGenerating(prev => ({ ...prev, [questionId]: false }));
      }, 300);
